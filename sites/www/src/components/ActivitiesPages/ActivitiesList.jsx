...
const ActivitiesList = () => {
  ...
  return (
    <Section>
      <ul className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-y-22 md:gap-y-28 lg:gap-y-32 gap-x-6">
              ...
              <div className="relative flex flex-col items-center">
                <div className="relative w-full overflow-hidden">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="w-full h-72 md:h-96 lg:h-128 object-cover"
                  />
                  {/* Overlay only on the image */}
                  <div className="absolute inset-0 bg-fourth-overlay pointer-events-none z-0"></div>
                </div>
