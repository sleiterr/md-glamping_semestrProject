...
export const getStayById = async (id) => {
  let result = {
    status: "error",
    message: `An Error Getting Stay occured`,
    data: [],
  };
    ...
    await stayModel
      ...
      .then((data) => {
        ...
        if (data.length === 0) {
          ...
          result = {
            status: "ok",
            message: "Stay fetched successfully",
            data: data,
          };
        }
      })
  ...
  try {
    ...
    body.image =
      body.image === undefined
        ? `${process.env.SERVER_HOST}/reviews/no-review.jpg`
        : body.image;
...
export const updateStay = async (body) => {
  ...
  try {
    ...
    let data = await stayModel.findByIdAndUpdate({ _id: body.id }, body, {
      new: true,
    });
